    ...
    <section className="pt-32 pb-20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-[0.9fr_1.1fr] gap-12 lg:gap-48 items-center">
        ...
        <div className="relative animate-float lg:scale-[1.35] xl:scale-[1.5] transition-transform duration-500 origin-center mt-12 lg:mt-0 pb-12 lg:pb-0">
          ...
          <div className="glass-card rounded-[40px] p-6 sm:p-8 aspect-square flex flex-col gap-6 relative overflow-hidden transition-all hover:shadow-2xl hover:-rotate-1">
            ...
            <div className="flex gap-1 bg-stone-100 p-1 sm:p-1.5 rounded-2xl">
              <button 
                onClick={() => setActiveTab("att")}
                className={`flex-1 flex items-center justify-center gap-2 py-2 sm:py-2.5 rounded-xl text-[10px] sm:text-sm font-bold transition-all ${activeTab === 'att' ? 'bg-white shadow-sm text-ink' : 'text-ink-3 hover:text-ink'}`}
              >
              ...
              <button 
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2 sm:py-2.5 rounded-xl text-[10px] sm:text-sm font-bold transition-all ${activeTab === 'tasks' ? 'bg-white shadow-sm text-ink' : 'text-ink-3 hover:text-ink'}`}
              >
              ...
              <button 
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2 sm:py-2.5 rounded-xl text-[10px] sm:text-sm font-bold transition-all ${activeTab === 'pdf' ? 'bg-white shadow-sm text-ink' : 'text-ink-3 hover:text-ink'}`}
              >
            ...
            <div className="flex-1 overflow-hidden">
              {activeTab === 'att' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                  {[
                    ...
                    <div key={i} className="bg-white/50 p-4 rounded-2xl border border-border-strong flex justify-between items-center">
                      <div className="flex-1">
                        <div className="font-bold text-sm mb-2">{s.n}</div>
                        <div className="h-1.5 w-full bg-stone-200 rounded-full overflow-hidden">
                          <div className={`h-full ${s.bg}`} style={{ width: `${s.p}%` }} />
                        </div>
                      </div>
              ...
              {activeTab === 'tasks' && (
                <div className="space-y-3 animate-in fade-in slide-in-from-right-4">
                  {[
                    ...
                    <div key={i} className="bg-white p-4 rounded-2xl border border-border-strong flex items-center justify-between">
                      <div>
                        <div className="font-bold text-sm mb-1">{t.t}</div>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${t.pc}`}>{t.p}</span>
                      </div>
                      <div className="text-[10px] font-bold text-ink-3 uppercase tracking-wider">{t.d}</div>
                    </div>
                  ))}
                </div>
              )}
              {activeTab === 'pdf' && (
                <div className="h-full flex flex-col gap-4 animate-in fade-in slide-in-from-right-4">
                  <div className="flex-1 border-2 border-dashed border-stone-200 rounded-3xl flex flex-col items-center justify-center bg-stone-50/50">
                    <FileText className="w-12 h-12 text-stone-300 mb-2" />
                    <p className="text-xs font-bold text-stone-400 uppercase tracking-widest">Drop PDFs here</p>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    {["Merge", "Split", "to Images", "to PDF"].map((m) => (
                      <div key={m} className="bg-white border border-border-strong p-3 rounded-xl text-center text-xs font-bold text-ink-2">
                        {m}
                      </div>
                    ))}
                  </div>
